...
def calc_iou(pred, label):
    intsec, union = calc_iou_np(pred, label)
    if union > 0:
        iou = intsec*1./union
    else:
        iou = 1.
...
def main():
    ...
    for line in lines:
        ...
        tot_frame += 1
        if last_video_id is None or last_video_id != video_id:    # for the first frame of a video
            images = []
            predictions = []
            features = []
            frame_ids = []
            ...
            # Keep the feature of first frame and its annotation
            ...
            # Write down the scoring of each frame
            sort_idx = np.argsort(sim_score).tolist()
            for k, tmp_idx in enumerate(sort_idx):
                select_f.write(
                    "%s,%s,%d,%s,%.2f\n" % (video_id, frame_id, k, frame_ids[tmp_idx], sim_score[tmp_idx]))
                if k > math.ceil(len(frame_ids) / 5.) + 3:      # Only write down the top-score frames
                    break
