...
def main():
    ...
    for line in lines:
        ...
        if last_video_id is None or last_video_id != video_id or last_obj_id != obj_id:
            ...
            if frame_id not in gt_objs.keys():
                gt_objs[frame_id] = {}
            gt_objs[frame_id][obj_id] = []
            ...
            gt_objs[frame_id][obj_id].append(np.array(last_prediction_ori))
            ...
            gt = label_loader(os.path.join(args.data_root, img_paths[1]))
            if frame_id not in predictions_objs.keys():
                predictions_objs[frame_id] = {}
            predictions_objs[frame_id][obj_id] = []
            if frame_id not in gt_objs.keys():
            ...
            gt_objs[frame_id][obj_id].append(np.array(gt))
            images.append(img)
