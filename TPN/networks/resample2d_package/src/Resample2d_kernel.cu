...
__global__ void kernel_Resample2d_updateOutput(const int n, const float* input1, const long4 input1_size, const long4 input1_stride,
    const float* input2, const long4 input2_size, const long4 input2_stride, float* output, const long4 output_size, const long4 output_stride, int sample, int kernel_size) {
    ...
    if(sample == 0) {
        ...
        int xf_int = max(min(int(round(xf)), dim_w-1), 0);
        int yf_int = max(min(int(round(yf)), dim_h-1), 0);
        val = DIM3_INDEX(input1, b, c, yf_int, xf_int);
    }
...
void Resample2d_kernel_forward(THCState* state, THCudaTensor* input1, THCudaTensor* input2, THCudaTensor* output, int sample, int kernel_size) {
    int n = 0;
    ...
    kernel_Resample2d_updateOutput<<< (n + CUDA_NUM_THREADS - 1)/CUDA_NUM_THREADS, CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state) >>>(
        ...
        THCudaTensor_data(state, output), output_size, output_stride, sample, kernel_size);

    THCudaCheck(cudaGetLastError());
}

void Resample2d_kernel_backward(THCState* state, THCudaTensor* input1, THCudaTensor* input2, THCudaTensor* gradOutput, THCudaTensor* gradInput1, THCudaTensor* gradInput2, int sample, int kernel_size) {
    int n = 0;
