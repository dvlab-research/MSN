...
class FlowNet2C(FlowNetC.FlowNetC):
    def __init__(self, args = None, batchNorm=False, div_flow=20, pre_norm=False):
    ...
    def forward(self, inputs):
        if self.pre_norm:
            ...
            for i in range(channels):
                j = i % 3
                inputs[:, i, :, :, :] = inputs[:, i, :, :, :] * self.std[j] + self.mean[j]
            inputs = inputs * self.rgb_max
        ...
        #else:
        return self.upsample1(flow2*self.div_flow), concat2
