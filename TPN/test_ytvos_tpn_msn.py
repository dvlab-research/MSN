...
from networks.select_pos_neg_firstann import vgg_flowc as selection
import cv2
...
def main():
    ...
    for line in lines:
        ...
        if len(img_paths) == 3:
            ...
            # extract vgg feature
            img_tensor = normalize(image2tensor(last_image, False), mean, std).unsqueeze(0).cuda()
            feat = selection_network.module.feat_extractor.forward(Variable(img_tensor).cuda().float()).data.cpu().numpy()
            features.append(feat)

            ann_tensor = image2tensor(last_prediction, True).unsqueeze(0).unsqueeze(1).cuda().float()
            ...
            # extract vgg feature
            img_tensor = normalize(image2tensor(img, False), mean, std).unsqueeze(0).cuda()
    ...
    #merge(args.predict_np_dir, dst_root=args.predict_dir)
